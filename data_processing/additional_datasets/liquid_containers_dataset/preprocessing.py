...
class DataSetFile:
    ...
    def __init__(self, fileName, isAormaly, affectedComponent, situation, operationalScenario,
                 combinedAffectedComponent='', combinedSituation=''):
...
def main(logsPath):
    ...
    filewriterCSV.writerow(
        ['IN3', 'IN2', 'IN1', 'IN0', 'PP', 'PG', 'VP', 'VG', 'R4', 'Slope', 'Anomaly', 'AffectedComponent', 'Situation',
         'OperationalScenario', 'CombinedAffectedComponent', 'CombinedSituation'])

    for x in records:
        read_file_and_write_rows(logsPath + '/' + x.fileName, x.is_anomaly, x.affected_component, x.situation,
                                 x.operational_scenario, x.combined_affected_component, x.combined_situation)
...
def read_file_and_write_rows(fileFullPath, isAnomaly, affectedComponent, situation, operationalScenario,
                             combinedAffectedComponent, combinedSituation):
    ...
    with open(fileFullPath, 'r') as file:
        ...
        formattedRow = [''] * 10
        formattedRow.extend([isAnomaly, affectedComponent, situation, operationalScenario, combinedAffectedComponent,
                             combinedSituation])
        ...
        for row in reader:
            if row[1] == "2":
                ...
                if dates_filled == 0:
                    ...
                    formattedRow = [''] * 10
                    formattedRow.extend(
                        [isAnomaly, affectedComponent, situation, operationalScenario, combinedAffectedComponent,
                         combinedSituation])
                    entries_count += 1
